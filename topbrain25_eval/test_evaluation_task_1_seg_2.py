...
def test_e2e_TopBrainEvaluation_Task_1_Seg_CT_2():
    ...
    # detection same as test_detection_single_label_ThresholdIoU
    assert (
        generated_metrics_json["case"]["all_detection_dicts"]["0"]["8"]["Detection"]
        == "TP"
    )
    assert (
        generated_metrics_json["case"]["all_detection_dicts"]["0"]["9"]["Detection"]
        == "FN"
    )
    assert (
        generated_metrics_json["case"]["all_detection_dicts"]["0"]["10"]["Detection"]
        == "TP"
    )
    assert (
        generated_metrics_json["case"]["all_detection_dicts"]["0"]["15"]["Detection"]
        == "FP"
    )
    # new in v2, contami metrics
    assert (
        generated_metrics_json["case"]["FGC_ratio_after_thresh"]["0"]
        == generated_metrics_json["case"]["FGC_sources_after_thresh"]["0"]
        == generated_metrics_json["case"]["UnderSeg_ratio_after_thresh"]["0"]
        == generated_metrics_json["case"]["UnderSeg_prevalence_after_thresh"]["0"]
        == generated_metrics_json["case"]["BGC_voxels"]["0"]
        == generated_metrics_json["case"]["BGC_sources"]["0"]
        == 0
    )
    ...
    # detection same as test_detection_sideroad_labels_Fig50_small_multiclass
    assert (
        generated_metrics_json["case"]["all_detection_dicts"]["1"]["9"]["Detection"]
        == "TN"
    )
    assert (
        generated_metrics_json["case"]["all_detection_dicts"]["1"]["10"]["Detection"]
        == "TP"
    )
    assert (
        generated_metrics_json["case"]["all_detection_dicts"]["1"]["15"]["Detection"]
        == "TP"
    )
    # new in v2, contami metrics
    assert (
        generated_metrics_json["case"]["FGC_ratio_after_thresh"]["1"]
        == generated_metrics_json["case"]["FGC_sources_after_thresh"]["1"]
        == generated_metrics_json["case"]["UnderSeg_ratio_after_thresh"]["1"]
        == generated_metrics_json["case"]["UnderSeg_prevalence_after_thresh"]["1"]
        == generated_metrics_json["case"]["BGC_voxels"]["1"]
        == generated_metrics_json["case"]["BGC_sources"]["1"]
        == 0
    )

    # aggregates
    # detection same as in test_aggregate_all_detection_dicts_Fig50ThresIoU.py
    assert generated_metrics_json["aggregates"]["dect_avg"]["precision"] == {
        "mean": 0.1388888888888889 * 18 / 15,
        "std": 0.3496029493900505,
    }
    assert generated_metrics_json["aggregates"]["dect_avg"]["recall"] == {
        "mean": 0.16666666666666666 * 18 / 15,
        "std": 0.4000000000000001,
    }
    assert generated_metrics_json["aggregates"]["dect_avg"]["f1_score"] == {
        "mean": 0.14814814814814814 * 18 / 15,
        "std": 0.36243347622889094,
    }
